...
def train_per_ticker(df, model):
    ...
    for ticker in tickers:
        ...
        for fold, (train_idx, test_idx) in enumerate(tscv.split(df_ticker)):
            ...
            report_summary.setdefault("lr", []).append({
                "fold": fold+1,
        ...
        with open(f"{REPORTS_PATH}/report_lr_{ticker}.json", "w") as f:
            json.dump(report_summary, f, indent=2)

if __name__ == "__main__":
    df = load_data()
    df, _ = encode_labels(df)
    train_per_ticker(df, MODEL)
